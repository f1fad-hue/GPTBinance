...
- Shows a two-decimal 1–5 portfolio rate using 20%/30%/50% weights for the 3/6/12-month signals and blending 40% broad with 60% allocation-correlated macro sentiment.
- Converts that continuous rate into a proportional 60%-80% SGOV minimum, rounded to 5%, then maximizes robust forecast growth above the cushion while preserving the 20% / 25% / 30% drawdown-cap bands.
...
- Exhaustively searches every feasible 5%-increment allocation above the macro cushion, keeps every required holding at 5% or more, caps BMNR at 10%, and maximizes net CAGR after a transparent forecast-uncertainty penalty.
...
- Runs every Saturday at 00:30 UTC (10:30 AM Australia/Brisbane) to refresh FRED macro data, extract official fund fees, scrub primary-source identity, map factual claims, and commit corrected data only when every automated check passes.
- Rechecks the repository schema, proportional-cushion math, exact active and scenario allocations, synchronized portfolio-path MDD, stress losses, 10,000-path correlated return/drawdown simulation, relevance statuses, required dashboard elements, Python and JavaScript syntax, and the weekend evidence report. A failed run retains its report, publishes no invalid data, and creates or updates one GitHub review issue; the issue closes automatically after a later clean run.
