...
async function main() {
  ...
  const dd = allocation.reduce((a, x) => a + x.weight * x.historicalDD / 100, 0);
  ...
  $('#slides').innerHTML = allocation.map(x => `<article class="slide"><span class="ticker">${x.ticker}</span><h3>${x.name}</h3><div class="metrics"><div><span>Historical maximum DD</span><b>${pct(x.historicalDD)}</b></div><div><span>Net CAGR input</span><b>${pct(x.grossCagr - x.fee)}</b></div><div><span>Fee</span><b>${pct(x.fee)}</b></div><div><span>Portfolio weight</span><b>${x.weight.toFixed(0)}%</b></div></div><p class="risk"><strong>Observed period:</strong> ${x.historicalDDStart} to ${x.historicalDDEnd}<br><strong>Peak → trough:</strong> ${x.historicalDDPeak} → ${x.historicalDDTrough}</p><p class="risk">Historical maximum DD is the sole optimizer and cap input; it is not a forecast.</p><p class="risk">${x.history}</p><p class="risk"><strong>Role:</strong> ${x.reason}</p></article>`).join('');

  const weightOf = ticker => allocation.find(x => x.ticker === ticker).weight.toFixed(0);
  $('#rationale').textContent = `The constrained max-growth model places ${weightOf('QQQ')}% in Nasdaq-100 growth, ${weightOf('IEMG')}% in emerging markets, ${weightOf('BINC')}% in flexible income, and ${weightOf('BMNR')}% in the digital-asset equity satellite. It maximizes stated net-CAGR assumptions subject to the ${rate.toFixed(1)}/5 macro signal and its ${cap}% historical maximum-drawdown cap. It is a transparent hypothetical allocation, not a recommendation or assurance of growth.`;
...
function optimize(assets, cap) {
  ...
  function search(i, left, weights) {
    if (i === assets.length - 1) {
      ...
      const drawdown = candidate.reduce((sum, weight, j) => sum + weight * assets[j].historicalDD / 100, 0);
